...
*Manuscript draft v0.7.1 — Genome Medicine submission target. 2026-04-16.*
*Renumbering from v0.7: main figures now Fig 1–9 matching the submission layout (Fig 4 = GSEA, Fig 5 = ML predictor including nested LOOCV ROC at 5B, Fig 6 = paired cascade with BCa forest at 6A, Fig 7 = external CD8-cytotoxic validation + Akiyoshi 2023, Fig 8 = HLA/neoantigen cascade, Fig 9 = clonal evolution). Supplementary figures numbered FigS1–FigS13 matching `genome_medicine_submission/supplementary_figures/`. Tables 1–3 + Tables S1–S9 now materialised as formal files. Sensitivity analyses (purity-adjusted paired Δ, BH FDR, drop-cohort meta) mentioned in Methods and Supp Text S4.*

---

## Abstract (347 words)

**Background.** Total neoadjuvant therapy (TNT) — long-course chemoradiation (CRT) plus FOLFOX/CAPOX — is standard for locally advanced rectal cancer (LARC). Modern post-consolidation complete-response rates are so high that a balanced "bad-responder" cohort cannot be accrued from post-consolidation tissue; paired-biopsy molecular studies are therefore tractable only around the **radiation (first) phase** of TNT, which is also a clinically actionable mid-treatment decision point. No molecular predictor of final TNT outcome from radiation-phase response is established for the MSS majority.

**Methods.** We profiled 35 MSS LARC patients by matched WES (77 samples; 41 tumor–normal pairs; 49 PASS Mutect2 VCFs) and RNA-seq (56 samples), with biopsies pre-CRT and post-CRT (before consolidation); sample flow is reconciled in CONSORT-style Supp Fig S12. Analyses: Mutect2 somatic calling, SigProfiler (COSMIC v3.3), msisensor-pro, CNVkit, OptiType, Bonferroni-corrected IMGT HLA-allelic imbalance, pVACseq-MHCflurry neoantigens, DESeq2, fgsea, ssGSEA, TRUST4, CMScaller. Thirty-seven per-subject features entered a LASSO classifier with nested outer-LOOCV and inner 5-fold hyperparameter tuning. Paired-cascade claims carry BCa 95 % bootstrap CIs (Table S8). External validation used 9 public GEO nCRT cohorts (N = 721) with per-cohort Dworak/Mandard/CAP/Rödel TRG mapping, aggregated by √N-weighted Stouffer's Z on seven pre-registered signatures. Purity-adjusted, BH-FDR, and leave-one-out sensitivity analyses are in Supp Text S4.

**Results.** All 41 matched tumors were MSS (max MSI 0.19 %, median 1.6 mut/Mb). **Narrative 1 — Pre-CRT predictor.** Hallmark GSEA: E2F targets (NES 2.78), G2M, MYC and Reactome DSB/HDR elevated in good responders (permutation P < 10⁻¹⁰); EMT down. **Nested outer-LOOCV LASSO AUC = 0.650 [0.45, 0.83]** (ElasticNet 0.686 [0.49, 0.85]); non-nested 0.755 reported alongside for transparency. **Narrative 2 — Radiation-induced cascade (exploratory, n = 14 paired).** Within-good changes were robust for SBS5 mutation clearance (Δ −76 [−145, −64]), MHC-I neoantigen clearance (Δ binders −312 [−626, −123]) and Treg infiltration (+1.26 [+0.34, +1.76]); **only Treg retained a between-group BCa CI excluding zero (P = 0.026)**. Bonferroni-strict HLA-LOH was seen in 2 good responders (subj 3, 4) — both resolved post-CRT. **External validation.** Across 9 cohorts (N = 721) the **CD8-cytotoxic axis reproduced robustly (Z = +2.74, P = 0.006, 8/9 concordant)**. Akiyoshi et al 2023 (GSE216616, n = 298) convergently reports OR 3.81 [1.82, 7.97] for cytotoxic lymphocytes, GZMA × PRF1 P = 0.005, and IFN-γ enrichment — total > 1,000 patients / 10 cohorts. Tumor-intrinsic DSB/HDR/E2F axes were cohort-heterogeneous (P > 0.19).
...
### 3.1 Cohort and study design (Fig 1; Table 1; Supp Fig S1, S12)
...
### 3.11 External validation — CD8-cytotoxic axis is reproducible across 9 nCRT cohorts (N = 721) (Fig 7, Table 3, Table S7)

On nine independent public GEO rectal cancer nCRT cohorts (N = 721) with harmonised per-cohort TRG-scale mapping (Table S7), meta-analysis (Stouffer's Z, √N-weighted, two-sided) of seven signatures yielded (Fig 7A):
...
## Figures (v0.7.1) — matching `genome_medicine_submission/main_figures/`
...
## Supplementary figures (v0.7.1) — matching `genome_medicine_submission/supplementary_figures/`
...
## Tables (v0.7.1) — materialised in `genome_medicine_submission/tables/`
...
*End of v0.7.1 — Genome Medicine target — 2026-04-16.*
