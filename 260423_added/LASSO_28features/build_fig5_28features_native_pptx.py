#!/usr/bin/env python3
"""
build_fig5_28features_native_pptx.py

Re-build of Figure 5 using **v1 master table** (37 cols) -> **28 numeric
features** that exactly match the `drop_cd8prolif_36` ablation scenario
fed to the nested-CV LASSO/ElasticNet pipeline (n_features=28 in
`nested_cv_drop_vs_swap.tsv`).

Why 28 (not 36, not 30)
-----------------------
- 37 columns in TableS1 master (clinical + WES + RNA composite + CMS + ID)
- drop 3 ID/label cols (subject_id, response_bin, response_num)
- drop 5 string categoricals (sex, cT, prepost_set, CMS, matched_wes)
- drop CD8_proliferation (cell-cycle contaminated; the winning ablation)
= 28 numeric features (with `age` retained as a numeric clinical feature).

This matches the model fit exactly. Previous Fig 5 (260418_add v2 master)
showed 30 features in Panels A/D because it used the post-hoc-extended
40-col master (with 3 added immune sigs) and additionally dropped age
- yielding a heatmap that did not match the ElasticNet model space.

Style is intentionally identical to 260418_add/18_fig5_native_pptx.py:
- 6.5x4.5 in slides, one panel per slide
- Arial throughout, no panel titles inside the plot
- 0.5-pt spines, 7-8pt tick labels, 9-10pt axis titles
- TNT palette: GOOD=#0a7d6e, BAD=#c53e1f
- gold #d4a300 highlight for winners / Youden / misclassified
- shadow-free shapes (empty <a:effectLst/>)

Panels
------
A  Feature correlation heatmap (28x28 Spearman)
B  Nested outer-LOOCV ROC (ElasticNet AUC 0.745, 95% CI 0.56-0.90)
C  4-feature coefficient forest (DSB +0.89, frac_amp -0.76, MHC II -0.23, MSI% +0.12)
D  UMAP of the 28-feature matrix, coloured by response
...
OUT = f"{ROOT}/260423_added/LASSO_28features"
DATA_V1 = f"{ROOT}/tables"                 # v1 master (37 cols, no immune sigs)
DATA_AUX = f"{ROOT}/260418_add"            # probs / feature_importance / nested CV table
os.makedirs(OUT, exist_ok=True)


# ---------------------------------------------------------------------------
# Style constants (project-wide TNT palette)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Load data — v1 master (37 cols) and the matching ablation winner row
# ---------------------------------------------------------------------------
master = pd.read_csv(f"{DATA_V1}/integrated_subject_master.tsv", sep="\t")
probs = pd.read_csv(f"{DATA_AUX}/probs_drop_cd8prolif_36_ElasticNet.tsv",
                    sep="\t")
feats4 = pd.read_csv(f"{DATA_AUX}/feature_importance_drop_cd8prolif.tsv",
                     sep="\t").sort_values("coef", ascending=False)
nested = pd.read_csv(f"{DATA_AUX}/nested_cv_drop_vs_swap.tsv", sep="\t")
...
assert N_EFFECTIVE == 28, f"Expected 28 features in winning model, got {N_EFFECTIVE}"

# 28-feature matrix --- exactly mirror the ablation pipeline
# drop 3 ID/labels + 5 string categoricals + CD8_proliferation = 9 cols
# leaves 28 numeric features (age retained)
drop_cols = ["subject_id", "response_bin", "response_num", "sex",
             "cT", "prepost_set", "CMS", "matched_wes",
             "CD8_proliferation"]
X_all = master.drop(columns=[c for c in drop_cols if c in master.columns],
                    errors="ignore")
...
assert N_FEAT == 28, f"Expected 28 features, got {N_FEAT}: {feat_names}"
print(f"28-feature matrix: {X_all.shape}; columns =")
for i, c in enumerate(feat_names, 1):
    print(f"  [{i:2d}] {c}")

# ---------------------------------------------------------------------------
# Panel A helper --- correlation matrix
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Panel D helper --- UMAP of the 28-feature matrix
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Panel E helper --- SHAP for the 4-feature ElasticNet
# ---------------------------------------------------------------------------
...
# ---------- Panel A: feature correlation heatmap (28x28) ----------
def build_A():
    s = new_slide(prs)
    ...
    GROUP_PAL = [
        ...
        ("Cell cycle", RGBColor(0xE3, 0x9A, 0x1E)),
        ("EMT / hypoxia", RGBColor(0x5A, 0x82, 0x61)),
        ("Immune", RGBColor(0x11, 0x8A, 0xB2)),
        ("Genomic", RGBColor(0x55, 0x55, 0x55)),
        ("Clinical", RGBColor(0x77, 0x55, 0x33)),
        ("Other", RGBColor(0xAA, 0xAA, 0xAA)),
    ]
    ...
    # Axis ticks + numeric feature-index labels (every 5, plus n)
    idx_ticks = [1, 5, 10, 15, 20, 25, n]
    idx_ticks = sorted(set(t for t in idx_ticks if 1 <= t <= n))
    for t in idx_ticks:
    ...
    add_text(s, hm_x, hm_y + hm_h + Inches(0.24),
             hm_w, Inches(0.2), f"Feature index (1–{n})",
    ...
    for feat, short in WINNING_FEATS.items():
        ...
        add_text(s, int(cx + cell_w / 2 - Inches(0.30)),
                 ...
                 align="center")
    legend_y = cb_y + cb_h + Inches(0.25) + len(GROUP_PAL) * Inches(0.14) + Inches(0.15)
    ...
    add_text(s, Inches(0.2), PLOT_Y + PLOT_H + Inches(0.6),
             ...
             f"{n} integrated numeric features fed to nested-CV LASSO "
             "(cell-cycle-contaminated CD8-proliferation removed; "
             "ID/label/categorical columns excluded; age retained); "
             "top / left strips mark feature group; gold borders mark "
             "the 4 LASSO-selected features.",
...
def build_B():
    ...
    AUC_FILL = RGBColor(0x9F, 0xC9, 0xBE)
    for i in range(len(base) - 1):
...
# ---------- Panel C: 4-feature coefficient lollipop ----------
def build_C():
    s = new_slide(prs)
...
def build_D():
    ...
    add_text(s, leg_x, leg_y + Inches(0.80),
             Inches(1.7), Inches(0.15),
             f"UMAP on {N_FEAT} features",
             size=6, color=RGBColor(0x55, 0x55, 0x55), align="left")
...
# ---------------------------------------------------------------------------
# Build all panels into one deck and save
# ---------------------------------------------------------------------------
build_A()
build_B()
build_C()
...
deck_path = f"{OUT}/Fig5_28features_native_editable.pptx"
prs.save(deck_path)
