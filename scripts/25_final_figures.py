...
# ---- Journal-style rcParams (Nature/Cell-like) ----
...
def _pl(ax, letter, x=-0.12, y=1.08):
...
gs = GridSpec(3, 3, figure=fig, hspace=0.95, wspace=0.50,
...
ax.pie(rcounts, labels=[f'{k} (n={v})' for k,v in rcounts.items()],
       colors=['#2a9d8f' if k=='good' else '#e76f51' for k in rcounts.index],
       autopct='%1.0f%%', textprops={'fontsize':7})
ax.set_title('Response distribution\n(TRG0-1 vs TRG2-3)', fontsize=9, fontweight='bold', pad=8)
_pl(ax, 'A', x=-0.10, y=1.12)
...
ax.set_ylabel('Percentage'); ax.legend(title='cT stage', fontsize=7, title_fontsize=7, loc='upper right')
ax.set_title('Clinical T stage by response\n(p=0.086)', fontsize=9, fontweight='bold', pad=8)
_pl(ax, 'B', x=-0.18, y=1.12)
...
ax.set_ylabel('Age (years)'); ax.set_title('Age \u00d7 sex (\u25cbM \u25b3F)', fontsize=9, fontweight='bold', pad=8)
_pl(ax, 'C', x=-0.18, y=1.12)
...
ax.set_xticklabels(subs, rotation=0, fontsize=6)
ax.set_yticklabels(ax.get_yticklabels(), fontsize=7)
ax.set_title('Sample matrix per subject (black=present)', fontsize=9, fontweight='bold', pad=8)
_pl(ax, 'D', x=-0.04, y=1.18)
...
ax.set_title('TMB pre (matched)   p=0.186', fontsize=9, fontweight='bold', pad=8)
_pl(ax, 'E', x=-0.18, y=1.12)
...
ax.set_title('MSI status   all <0.2% (MSS)', fontsize=9, fontweight='bold', pad=8)
_pl(ax, 'F', x=-0.18, y=1.12)
...
sns.stripplot(data=neo_pre, x='response_bin', y='n_sites_with_binder', ax=ax, order=['good','bad'],
              color='black', size=4)
ax.set_title('Neoantigen sites (pre)\ngood 73 vs bad 66, p=0.082', fontsize=9, fontweight='bold', pad=8)
ax.set_ylabel('Sites with MHC-I binder (<500nM)')
_pl(ax, 'G', x=-0.22, y=1.12)

plt.suptitle('Figure 1. Cohort overview and basic molecular landscape',
             fontsize=11, fontweight='bold', y=0.99)
plt.savefig(FIG/'Fig1_cohort.png', dpi=600, bbox_inches='tight')
...
for _i, (ax, metric, label) in enumerate(zip(axes[0], ['IGH_n','IGH_shannon','TRB_shannon','TRB_n'],
    ...
    ax.set_title(f'{label}', fontsize=9, fontweight='bold', pad=8)
    ax.text(0.98, 0.97, f'P = {p:.3f}', transform=ax.transAxes,
            ha='right', va='top', fontsize=7, color='#222',
            bbox=dict(boxstyle='round,pad=0.2', fc='white', ec='none', alpha=0.8))
    ax.set_xlabel('')
    _pl(ax, _letters[_i], x=-0.22, y=1.12)

# Bottom row: neoantigen delta
for _j, (ax, metric, label) in enumerate(zip(axes[1][:4], ['delta_binders','delta_sites','delta_strong','delta_PCN'],
    ...
    ax.text(0.98, 0.97, f'P = {p:.3f}', transform=ax.transAxes,
            ...
            bbox=dict(boxstyle='round,pad=0.2', fc='white', ec='none', alpha=0.8))
    ax.set_xlabel('')
    _pl(ax, _letters[4+_j], x=-0.22, y=1.12)

fig.suptitle('Figure 6. Treatment-induced immune activation and neoantigen clearance in good responders',
             fontsize=11, fontweight='bold')
plt.savefig(FIG/'Fig6_paired_delta.png', dpi=600, bbox_inches='tight')
