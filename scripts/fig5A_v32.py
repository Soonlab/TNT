"""
Fig 5A v3.2 — clean layout with:
  - y-labels in dedicated column (no overlap with color strip)
  - explicit gaps between heatmap / colorbar / legend
"""
...
# ============================================================
# Redesigned layout:
#   Rows: [top-label gap: 0.12, top color bar: 0.14, heatmap: 8, x-label gap: 1.6]
#   Cols: [y-label: 2.0, left color bar: 0.14, heatmap: 8, gap: 0.25, colorbar: 0.18, gap: 0.35, legend: 1.7]
# ============================================================
fig = plt.figure(figsize=(14, 10))
gs = fig.add_gridspec(4, 7,
    height_ratios=[0.12, 0.14, 8, 1.8],
    width_ratios=[2.0, 0.14, 8, 0.35, 0.18, 0.35, 1.7],
    hspace=0.04, wspace=0.04)

top_colors = [OMIC_COLORS[omic_map[f]] for f in ordered_features]

# Top color bar (row 1, col 2)
...
# Y-label column (row 2, col 0) — dedicated text-only axis
...
# Colorbar (col 4, row 2 top portion)
ax_cb_holder = fig.add_subplot(gs[2, 4])
ax_cb_holder.axis('off')
cbar_ax = inset_axes(ax_cb_holder, width="100%", height="35%", loc='upper center',
                     bbox_to_anchor=(0, 0, 1, 1), bbox_transform=ax_cb_holder.transAxes)
...
# Legend column (col 6, row 2)
ax_leg = fig.add_subplot(gs[2, 6])
ax_leg.axis('off')
ax_leg.text(0.0, 0.99, 'Omic group', fontsize=10.5, color='#0e2a47', fontweight='bold',
...
for i, og in enumerate(omic_present):
    y_pos = 0.92 - i*y_step
    ax_leg.add_patch(Rectangle((0.02, y_pos-0.025), 0.20, 0.05, color=OMIC_COLORS[og],
                                transform=ax_leg.transAxes))
    ax_leg.text(0.26, y_pos, og.replace('_',' '), transform=ax_leg.transAxes,
                fontsize=9.5, va='center', color='#0e2a47')

save_panel(fig, 'Fig5A_correlation', OUT)
print('=== Fig 5A v3.2 saved ===')
