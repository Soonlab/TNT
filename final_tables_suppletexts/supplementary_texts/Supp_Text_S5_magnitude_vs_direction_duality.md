# Supplementary Text S5 — Magnitude versus directional-consistency test duality

This supplementary text documents the magnitude-versus-direction test duality that motivates reporting both Wilcoxon / Mann-Whitney on Δ magnitude and binomial sign / Fisher on up-down counts for every paired test in the manuscript. A worked example on the IGHV panel (IGHV3-7, IGHV3-74, IGHV6-1) is provided to illustrate why for small-N paired designs (n = 6 + 6 here) both tests must be reported. Source: `260418_add/PHARMACODYNAMICS_AND_DIRECTIONAL_CONSISTENCY.md`.
